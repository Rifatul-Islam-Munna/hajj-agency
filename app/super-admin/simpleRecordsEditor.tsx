...
function blank(collection: string): ContentRecord {
  return { id: 0, collection_key: collection, slug: "", title: "", subtitle: "", content: "", image_url: "", icon_url: "", link_text: "", link_url: "", button_bg_color: "", button_hover_color: "", social_facebook: "", social_x: "", social_youtube: "", enabled: true, sort_order: 0 };
}
...
function RecordFields({ item, onChange }: { item: ContentRecord; onChange: (update: Partial<ContentRecord>) => void }) {
  ...
  if (audio) return <div className="admin-fields"><Field label="Audio title" value={item.title} onChange={(title) => onChange({ title })} /><Field label="Slug" value={item.slug} onChange={(slug) => onChange({ slug })} /><Field label="Audio URL" value={item.link_url} onChange={(link_url) => onChange({ link_url })} placeholder="https://example.com/audio.mp3" /><Field label="Sort order" type="number" value={String(item.sort_order)} onChange={(value) => onChange({ sort_order: Number(value) || 0 })} /><div className="admin-field full"><label className="admin-toggle"><input type="checkbox" checked={item.enabled} onChange={(event) => onChange({ enabled: event.target.checked })} /> Published</label></div></div>;
  return <div className="admin-fields"><Field label={testimonial ? "Right side title" : "Title"} value={item.title} onChange={(title) => onChange({ title })} /><Field label="Slug" value={item.slug} onChange={(slug) => onChange({ slug })} /><Field label={testimonial ? "Right side small text" : "Subtitle / role"} value={item.subtitle} onChange={(subtitle) => onChange({ subtitle })} /><Field label="Sort order" type="number" value={String(item.sort_order)} onChange={(value) => onChange({ sort_order: Number(value) || 0 })} /><RichTextEditor label={testimonial ? "Testimonial quote" : "Description / answer"} value={item.content} onChange={(content) => onChange({ content })} /><ImageUploadField label={slider ? "Slider background image" : testimonial ? "Testimonial slider image" : "Main image"} value={item.image_url} onChange={(image_url) => onChange({ image_url })} recommended={slider ? "1920 x 900 px" : testimonial ? "900 x 700 px" : "900 x 650 px"} help={testimonial ? "Shows as the big left image and the small round image for this testimonial slide." : undefined} /><ImageUploadField label={slider ? "Right side slider image" : "Icon image"} value={item.icon_url} onChange={(icon_url) => onChange({ icon_url })} recommended={slider ? "760 x 760 px transparent PNG/WebP" : "Transparent PNG/SVG"} help={slider ? "Shows on the right side. It is fitted to the slider height automatically." : undefined} /><Field label="Button text" value={item.link_text} onChange={(link_text) => onChange({ link_text })} /><Field label="Button URL" value={item.link_url} onChange={(link_url) => onChange({ link_url })} /><ColorField label="Button background color" value={item.button_bg_color} onChange={(button_bg_color) => onChange({ button_bg_color })} /><ColorField label="Button hover color" value={item.button_hover_color} onChange={(button_hover_color) => onChange({ button_hover_color })} /><Field label="Facebook URL" value={item.social_facebook} onChange={(social_facebook) => onChange({ social_facebook })} /><Field label="X URL" value={item.social_x} onChange={(social_x) => onChange({ social_x })} /><Field label="YouTube URL" value={item.social_youtube} onChange={(social_youtube) => onChange({ social_youtube })} /><div className="admin-field full"><label className="admin-toggle"><input type="checkbox" checked={item.enabled} onChange={(event) => onChange({ enabled: event.target.checked })} /> Published</label></div></div>;
}
