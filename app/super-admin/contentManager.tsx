...
export default function ContentManager({ initialRecords }: { initialRecords: ContentRecord[] }) {
  ...
  return <div>
    ...
    <div className="admin-package-list">{visible.map((item) => <div className="admin-package-card" key={item.id}><div className="admin-section-head"><div style={{ display: "flex", alignItems: "center", gap: 14 }}>{(item.image_url || item.icon_url) && <img className="admin-package-preview" src={item.image_url || item.icon_url} alt={item.title} />}<div><h3>{item.title}</h3><p className="admin-subtitle">{item.collection_key} / {item.slug}</p></div></div><label className="admin-toggle"><input type="checkbox" checked={item.enabled} onChange={(event) => patch(item.id, { enabled: event.target.checked })} /> Enabled</label></div><RecordFields item={item} onChange={(update) => patch(item.id, update)} /><div className="admin-actions" style={{ marginTop: 16 }}><button className="admin-button" disabled={busy === item.id} onClick={() => save(item)}><Save size={17} /> Save</button>{detailUrl(item) && <a className="admin-button secondary" href={detailUrl(item)} target="_blank"><ExternalLink size={17} /> Preview</a>}<button className="admin-button danger" disabled={busy === item.id} onClick={() => remove(item)}><Trash2 size={17} /> Delete</button></div></div>)}</div>
  </div>;
}
...
function RecordFields({ item, onChange }: { item: ContentRecord; onChange: (update: Partial<ContentRecord>) => void }) {
  ...
  const prayer = item.collection_key === "prayer-times";
  const stats = item.collection_key === "statistics";
  const slider = item.collection_key === "slider";
  const faq = item.collection_key === "faq";
  return <div className="admin-fields" style={{ marginTop: 16 }}>
    <Field label={faq ? "Question" : prayer ? "Prayer name" : stats ? "Counter label" : footer ? "Footer link text" : "Title"} value={item.title} onChange={(title) => onChange({ title })} />
    <Field label="Slug" value={item.slug} onChange={(slug) => onChange({ slug })} />
    {!footer && <Field label={prayer ? "Azan time" : stats ? "Counter number" : slider ? "Small heading" : "Subtitle / role"} value={item.subtitle} onChange={(subtitle) => onChange({ subtitle })} />}
    <Field label="Sort order" value={String(item.sort_order)} type="number" onChange={(value) => onChange({ sort_order: Number(value) || 0 })} />
    {!footer && !prayer && !stats && <RichTextEditor label={faq ? "Answer" : "Description / details"} value={item.content} onChange={(content) => onChange({ content })} help="Supports paragraphs, line breaks, headings, bold text, lists, quotes and links." />}
    {!footer && !prayer && !stats && !faq && <ImageUploadField label={slider ? "Slide background" : "Main image"} value={item.image_url} onChange={(image_url) => onChange({ image_url })} recommended={imageSize(item.collection_key)} />}
    {(item.collection_key === "services" || stats || slider) && <ImageUploadField label={slider ? "Slide foreground image" : stats ? "Counter icon" : "Service icon"} value={item.icon_url} onChange={(icon_url) => onChange({ icon_url })} recommended={slider ? "900 × 900 px transparent" : "160 × 160 px transparent"} />}
    {(footer || slider || ["services", "pillars", "guides"].includes(item.collection_key)) && <Field label={footer ? "Link URL" : "Custom link URL"} value={item.link_url} onChange={(link_url) => onChange({ link_url })} help={footer || slider ? "Enter the destination URL." : "Leave empty to use the connected detail page."} />}
    {!footer && <Field label={prayer ? "Iqamah time" : stats ? "Counter suffix" : "Link button text"} value={item.link_text} onChange={(link_text) => onChange({ link_text })} />}
...
function detailUrl(item: ContentRecord) { return item.link_url || (["services", "pillars", "guides"].includes(item.collection_key) ? `/content/${item.collection_key}/${item.slug}` : ""); }
function imageSize(collection: string) { return collection === "slider" ? "1920 × 900 px" : collection === "guides" ? "800 × 900 px" : collection === "testimonials" ? "400 × 400 px" : collection === "pillars" ? "600 × 600 px" : "900 × 650 px"; }
