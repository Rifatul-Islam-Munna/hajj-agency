import { BookOpenText, Boxes, Database, Gauge, Images, Inbox, LayoutTemplate, Settings } from "lucide-react";
      ...
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <div className="admin-nav-group">
            ...
            <Link href="/super-admin/content"><Database size={17} /> Connected Content</Link>
            <Link href="/super-admin/contacts"><Inbox size={17} /> Contact Enquiries</Link>
            <Link href="/super-admin/settings"><Settings size={17} /> Site & Navbar</Link>
            <Link href="/super-admin/footer-contact"><Images size={17} /> Footer & Contact</Link>
          </div>
          <div className="admin-nav-group">
            <div className="admin-nav-label">Pages and sections</div>
            {CMS_PAGES.map((page) => <Link key={page.slug} href={`/super-admin/pages/${page.slug}`}><LayoutTemplate size={16} /> {page.name}</Link>)}
          </div>
        </nav>
      </aside>
      <main className="admin-main">
        <header className="admin-topbar"><div className="admin-topbar-user">Signed in as <strong>{session.name}</strong></div><LogoutButton /></header>
