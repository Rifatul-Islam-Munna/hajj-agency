...
export default function PackageBookingPanel({ bundle }: { bundle: PackageBookingBundle }) {
  ...
  return <div className="package-booking-panel">
    ...
    <div className="d-grid gap-2"><button className={`green_btn ${buttonClass}`} style={buttonStyle} type="button" onClick={() => selectPackage(true)}><span>Book Now</span></button><button className={`green_border_btn ${buttonClass}`} style={buttonStyle} type="button" onClick={() => selectPackage(false)}><ShoppingCart size={17} /> Add to Cart</button></div>
  </div>;
}
