...
export function CmsSection({ sectionKey, children }: { sectionKey: string; children: ReactNode }) {
  ...
  const section = useMemo(
    () => page?.sections.find((item) => item.section_key === sectionKey) || null,
    [page, sectionKey],
  );
  ...
  useLayoutEffect(() => {
    ...
    applySectionContent(ref.current, section);
  }, [section]);
  ...
  return (
    <div ref={ref} className="cms-section-wrapper" data-cms-section={sectionKey}>
      {children}
    </div>
  );
}
