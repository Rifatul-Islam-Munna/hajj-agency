...
export default function SliderClient({ slides }: { slides: ContentRecord[] }) {
  return (
    <section className="home-slider position-relative"><Swiper modules={[Navigation]} loop={slides.length > 1} navigation={{ prevEl: ".rev_prev", nextEl: ".rev_next" }} className="home-slider-active">
      {slides.map((slide) => <SwiperSlide key={slide.id}><div className="single-slide" style={{ backgroundImage: `url(${slide.image_url})` }}><div className="container"><div className="row position-relative"><div className="col-xl-6 col-12"><div className="slider-content"><span className="slider_subtitle animated">{slide.subtitle}</span><h1 className="animated">{slide.title}</h1><div className="animated cms-rich-content" dangerouslySetInnerHTML={{ __html: slide.content }} />{slide.link_url && <Link href={slide.link_url} className="yellow_btn animated"><span>{slide.link_text || "Discover More"}</span></Link>}</div></div>{slide.icon_url && <div className="hslider_image"><img src="/assets/img/slider/shape.png" className="slider_shape animated" alt="" /><img src={slide.icon_url} className="slider_img animated" alt={slide.title} /></div>}</div></div></div></SwiperSlide>)}
